...
class TelegramBot:
    ...
    def handle_stop_script(self, message):
        ...
        if len(parts) < 2:
            ...
            try:
                ...
                self.bot.send_message(message.chat.id, f"✅ La sessione screen con ID {screen_id} è stata fermata.")
            except subprocess.CalledProcessError:
                print(f"⚠️ Impossibile fermare la sessione screen con ID {screen_id}.")
                self.bot.send_message(message.chat.id, f"⚠️ Impossibile fermare la sessione screen con ID {screen_id}.")
    ...
    def handle_screen_status(self, message):
        ...
        screen_id = message.text.split()[1]
        try:
            subprocess.check_output(["screen", "-S", screen_id, "-X", "quit"])
            print(f"✅ La sessione screen con ID {screen_id} è stata fermata.")
            self.bot.send_message(message.chat.id, f"✅ La sessione screen con ID {screen_id} è stata fermata.")
    ...
    def handle_response(self, message):
        if message.text == "stop":
            self.bot.send_message(message.chat.id, "Script fermato.")
        else:
